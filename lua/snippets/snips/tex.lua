...
local function environment(trigger, name)
  return s({ trig = "^(%s*)" .. trigger, regTrig = true }, {
    f(regex_cap_group(1), {}),
    t({ "\\begin{" .. name .. "}", "  " }),
    f(regex_cap_group(1), {}),
    i(0),
...
local function ref(trigger, name)
  ...
  return s("ref" .. trigger, {
    t(name .. "~" .. command .. "{" .. trigger .. ":"),
    i(1),
    t("}"),
    i(0),
  })
...
local function command(args)
  ...
  local condition = args.condition or function()
    return true
  end
  local snip
  if args.noargs then
...
local function wrap(args)
  ...
  return s({ trig = "([^${} ]+)" .. trigger, regTrig = true }, {
    t("\\" .. command .. "{"),
    f(regex_cap_group(1), {}),
    t("}"),
    i(0),
...
local function imap(trigger, command)
  return s({ trig = "´" .. trigger, wordTrig = false }, {
    t("\\" .. command),
    i(0),
  }, { condition = ismath })
end
